...
chunk-reader = proc(file offset)
	thunk = defer(call(proc()
		_ = print('-> reading from offset: ' offset)
		call(chunk-reader file plus(offset 16))
	end))

	list(
		call(stdfiles.read-at file 16 offset)
		proc() force(thunk) end
	)
end
...
main = proc()
	...
	# NOTE: now file is closed so reading from there
	# would fail but reading value from thunk succeeds
	_ = call(stdfiles.close file)

	chunk-5 stream-5 = call(stream-2):
	...
	plus('\nFile in chunks:\n' call(stdpp.pform
		list(
			...
			chunk-5 # read from thunk (memoization)
		)
	))
end
